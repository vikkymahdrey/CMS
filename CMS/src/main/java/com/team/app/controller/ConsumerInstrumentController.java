...
public class ConsumerInstrumentController {
	...
	@SuppressWarnings("unchecked")
	@RequestMapping(value = "/cmsInfo", method = RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<String> parkingInfoHandler(){
		...
		try{			
			...
			List<LoraFrame> frmList=mqttFramesService.devEUIGroupBy();
				...
				if(frmList!=null && !frmList.isEmpty()){
					...
					JSONObject result=null;
						result=new JSONObject();					
					for(LoraFrame f : frmList){
						List<LoraFrame> fList=mqttFramesService.getFrameByDevEUI(f.getDevEUI());
						if(fList!=null && !fList.isEmpty()){
							LoraFrame frm=fList.get(0);
							JSONObject json=null;
								json=new JSONObject();	
								json.put("nodeName", frm.getNodeName());
								json.put("cattle1", frm.getLed1());
								json.put("cattle2", frm.getLed2());
								json.put("cattle3", frm.getLed3());
								json.put("cattle4", frm.getLed4());
